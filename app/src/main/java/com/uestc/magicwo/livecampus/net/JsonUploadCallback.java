 ...
 */
public abstract class JsonUploadCallback<T> extends AbsCallback<T> {
...
//                .params("token", "3215sdf13ad1f65asd4f3ads1f");
    ...
    public T convertSuccess(Response response) throws Exception {
        ...
        if (typeArgument == Void.class) {
            ...
            SimpleUploadResponse simpleUploadResponse = Convert.fromJson(jsonReader, SimpleUploadResponse.class);
            response.close();
            //noinspection unchecked
            return (T) simpleUploadResponse.toBaseUploadResponse();
        } else if (rawType == BaseUploadResponse.class) {
            //有数据类型，表示有data
            BaseUploadResponse baseUploadResponse = Convert.fromJson(jsonReader, type);
            response.close();
            if(baseUploadResponse.isSuccess()){
                return (T) baseUploadResponse;
            }
